...
///
/// Implementations should (if possible) hold that `Type::from_networking(obj.networking_bytes())
/// == obj`.
///
...
impl Network for UpdateMessage {
    ...
    fn from_networking(mut bytes: &[u8]) -> Result<Self, NetErr> {
        ...
        let size = label_s[0].try_into().unwrap();
        // attempt to read `size` bytes to buffer
        // I'm fairly confident that a u8 fits into a usize
        label_v = Vec::with_capacity(size);
        let read_size = bytes.read(&mut label_v)?;
        if read_size < size {
            return Err(NetErr::NotEnoughData);
        }
        bytes.read_exact(&mut value_s)?;
        let size = value_s[0].try_into().unwrap();
        value_v = Vec::with_capacity(size);
